    ...
    <div className={`flex my-2 ${isOutgoing ? 'justify-end' : 'justify-start'} group items-center`}>
      ...
      <div
        className={`max-w-sm lg:max-w-lg px-5 py-3 rounded-3xl shadow-sm ${isOutgoing
          ...
          : 'bg-gray-100 text-accent rounded-bl-lg'
          }`}
      >
        {message.attachment && (
        ...
        <p className={`text-xs mt-1.5 ${isOutgoing ? 'text-green-100' : 'text-accent'} text-right`}>
          {message.time}
        </p>
      </div>
    </div>
...
const ChatSection = ({ user }) => {
  ...
  return (
    <div className="flex h-full font-sans bg-gradient-to-br from-blue-100 to-purple-100 text-gray-800">
      <aside className="w-full md:w-80 lg:w-96 themed-sidebar backdrop-blur-lg border-r border-gray-200/80 flex flex-col">
        <div className="p-4 border-b border-gray-200/80">
          <div className="relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
            <input type="text" placeholder="Search contacts..." className="w-full pl-12 pr-4 py-3 bg-gray-100 border-transparent rounded-xl focus-accent focus:bg-white transition-all" />
          </div>
        </div>
      ...
      <main className="hidden md:flex flex-1 flex-col bg-transparent relative overflow-hidden">
        <header className="bg-white/80 backdrop-blur-lg border-b border-gray-200/80 p-4 flex items-center justify-between shadow-sm relative z-10">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setActiveCall('voice')} className="p-3 text-gray-500 hover:bg-gray-50 hover:text-accent rounded-full transition-colors"><Phone size={20} /></button>
            <button onClick={() => setActiveCall('video')} className="p-3 text-gray-500 hover:bg-gray-50 hover:text-accent rounded-full transition-colors"><Video size={20} /></button>
            <div className="relative">
              <button onClick={() => setShowMoreOptions(!showMoreOptions)} className="p-3 text-gray-500 hover:bg-gray-50 hover:text-accent rounded-full transition-colors"><MoreVertical size={20} /></button>
              {showMoreOptions && (
        ...
        <footer className="bg-white/80 backdrop-blur-lg border-t border-gray-200/80 p-4">
          ...
          <form onSubmit={handleSendMessage} className="flex items-center gap-4">
            ...
            <input type="text" value={inputText} onChange={handleInputChange} placeholder="Type a message..." className="flex-1 px-5 py-3 bg-gray-100 border-transparent rounded-full focus-accent" />
            <button type="submit" className="p-3 bg-accent text-white rounded-full hover:opacity-95 transition-colors disabled:bg-gray-300 shadow-lg shadow-accent/30" disabled={!inputText.trim() && !attachment}><Send size={20} /></button>
          </form>
        </footer>
      </main>
    </div>
...
/* Using shared SimplePieChart component */

/* eslint-disable-next-line no-unused-vars */
const _SimpleLineChart = ({ data, dataKey, xAxisKey, color }) => {
...
const ParentDashboard = ({ onLogout }) => {
  ...
  const [, _setProfileLoading] = useState(false);
  ...
  const [attendanceActiveIndex, setAttendanceActiveIndex] = useState(null);
  const [feeActiveIndex, setFeeActiveIndex] = useState(null);
  const [gradeActiveIndex, setGradeActiveIndex] = useState(null);
  const [busLocation, setBusLocation] = useState({
    lat: 12.9716,
    lng: 77.5946,
    status: 'Moving',
    speed: '45 km/h',
    nextStop: 'Market Road',
    eta: '10 mins',
  ...
  // Mock route path for demo
  const routePath = [
    [12.9716, 77.5946], [12.9720, 77.5950], [12.9730, 77.5960],
    [12.9740, 77.5970], [12.9750, 77.5980], [12.9760, 77.5990],
    [12.9770, 77.6000], [12.9780, 77.6010], [12.9790, 77.6020], [12.9800, 77.6030]
  ];

  useEffect(() => {
    if (activeSection === 'Bus Tracker') {
      ...
      const handleLocationUpdate = (data) => {
        if (data.location) {
          setBusLocation(prev => ({
            ...
            speed: '45 km/h', // Mock speed if not provided
            lastUpdated: 'Just now'
          }));
        }
      };
      ...
      // Fallback simulation if no socket data
      const interval = setInterval(() => {
        // Only simulate if we haven't received socket data recently (optional logic)
        // For now, keeping the simulation as a fallback or demo driver
      }, 2000);
  ...
  // Listen for bus proximity alerts
  useEffect(() => {
    const handleBusAlert = (data) => {
    ...
    socket.on('bus-proximity', handleBusAlert);
    return () => socket.off('bus-proximity', handleBusAlert);
  }, []);

  // Fetch user home location
  useEffect(() => {
    if (user && user.homeCoordinates) {
      setHomeLocation(user.homeCoordinates);
    }
  }, [user]);

  const handleSetHomeLocation = async (latlng) => {
    setHomeLocation({ lat: latlng.lat, lng: latlng.lng });
    setIsSettingHome(false);

    try {
      const token = localStorage.getItem('token');
      await fetch(`http://localhost:5000/api/users/${user.id}/home-location`, {
        method: 'PATCH',
        headers: {
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      setUser(currentUser);
      fetchChildrenDetails(currentUser);
    } else {
      setLoading(false);
    }
  }, []);
  ...
  const fetchChildrenDetails = async (parentUser) => {
    try {
      ...
      if (data.success && parentUser.children) {
        ...
        // Fallback if API returns failure or no children data structure match
        if (parentUser.children && parentUser.children.length > 0) {
          const fallbackChildren = parentUser.children.map(child => ({
            id: `STU-${Math.floor(100000 + Math.random() * 900000)}`,
            name: child.name,
            grade: child.grade || child.className || 'N/A'
          }));
          setChildren(fallbackChildren);
          if (fallbackChildren.length > 0) setSelectedChildId(fallbackChildren[0].id);
        }
      }
    } catch (error) {
  ...
  const FEE_COLORS = ["#3b82f6", "#f59e0b", "#8b5cf6", "#ec4899"];
  ...
  const GRADE_COLORS = ["#8b5cf6", "#6366f1", "#3b82f6"];

  const NavItem = ({ icon, label, active, onClick }) => (
    <button
      onClick={onClick}
      className={`flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-200 group w-full text-left ${active ? 'bg-white/20 text-white font-bold' : 'text-white/70 hover:bg-white/10 hover:text-white font-medium'}`}
    >
      <span className={`${active ? 'text-white' : 'text-white/70 group-hover:text-white transition-colors'}`}>{icon}</span>
      <span>{label}</span>
    </button>
  );
  ...
  return (
    <div className="flex min-h-screen bg-[#FAFAF3] font-sans text-gray-800 overflow-hidden">
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#556B2F] border-r border-olive-700 flex flex-col transition-transform duration-300 ease-in-out shadow-sm ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        ...
        <div className="p-4 border-t border-gray-100">
          <button
            ...
            className="flex items-center gap-3 text-white/80 hover:text-red-200 hover:bg-red-900/20 w-full p-3 rounded-xl transition-colors duration-200"
          >
            <LogOut size={20} />
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
          ...
          <div className="flex items-center gap-6">
            <div className="relative">
              <button
                ...
                className={`relative p-2 rounded-full transition-colors ${showNotifications ? 'bg-indigo-100 text-indigo-600' : 'text-gray-600 hover:bg-gray-100'}`}
              >
                <Bell size={24} />
              ...
              {showNotifications && (
                <div className="absolute top-12 right-0 w-80 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50 animate-fadeIn">
                  <div className="px-4 py-2 border-b border-gray-50 flex justify-between items-center">
                    ...
                    <button onClick={markAllAsRead} className="text-xs text-indigo-600 hover:underline">Mark all read</button>
                  </div>
        ...
        {/* Content Body */}
        <main className={`flex-1 ${activeSection === 'Chats' ? 'overflow-hidden' : 'overflow-y-auto p-8'} themed-bg animate-fadeIn`} key={activeSection}>
          {activeSection !== 'Chats' && (
            <div className="max-w-7xl mx-auto space-y-8">
              ...
              {activeSection === 'Dashboard' && (
                <>
                      ...
                      <div className="h-56 flex items-center">
                        ...
                        <div className="w-32 pl-2">
                          <ul className="space-y-1">
                            {attendanceOverviewData.map((d, idx) => (
                              <li key={idx} className={`text-xs flex items-center justify-between p-1 rounded cursor-pointer ${attendanceActiveIndex === idx ? 'bg-gray-50' : ''}`} onMouseEnter={() => setAttendanceActiveIndex(idx)} onMouseLeave={() => setAttendanceActiveIndex(null)}>
                                <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full" style={{ backgroundColor: ATTENDANCE_COLORS[idx] }}></span>{d.label}</div>
                      ...
                      <div className="h-56 flex items-center">
                        <div className="flex-1 h-full flex items-center justify-center">
                          <SimplePieChart
                            ...
                            colors={FEE_COLORS}
                            activeIndex={feeActiveIndex}
                            onHover={setFeeActiveIndex}
                          />
                        </div>
                        <div className="w-32 pl-2">
                          <ul className="space-y-1">
                            {feeBreakdownData.map((d, idx) => (
                              <li key={idx} className={`text-xs flex items-center justify-between p-1 rounded cursor-pointer ${feeActiveIndex === idx ? 'bg-gray-50' : ''}`} onMouseEnter={() => setFeeActiveIndex(idx)} onMouseLeave={() => setFeeActiveIndex(null)}>
                                <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full" style={{ backgroundColor: FEE_COLORS[idx % FEE_COLORS.length] }}></span>{d.label}</div>
                                <span className="font-bold">{d.value}%</span>
                              </li>
                            ))}
                          </ul>
                  ...
                  <div className="bg-white p-6 rounded-2xl shadow-sm mb-6 hover-card">
                    ...
                    <div className="grid grid-cols-7 gap-1">
                      ...
                      {Array.from({ length: getDaysInMonth(calendarDate) }, (_, i) => {
                        ...
                        else if (events.some(e => e.type === 'exam')) bgClass = 'bg-gray-50 text-accent font-medium';
                  ...
                  <div className="bg-indigo-600 rounded-2xl p-6 text-white flex justify-between items-center shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                    <div>
                    ...
                    <div className="h-56 flex items-center">
                      <div className="flex-1 h-full flex items-center justify-center">
                        <SimplePieChart
                          ...
                          activeIndex={gradeActiveIndex}
                          onHover={setGradeActiveIndex}
                        />
                      </div>
                      <div className="w-32 pl-2">
                        <ul className="space-y-1">
                          {gradeDistributionData.map((d, idx) => (
                            <li key={idx} className={`text-xs flex items-center justify-between p-1 rounded cursor-pointer ${gradeActiveIndex === idx ? 'bg-gray-50' : ''}`} onMouseEnter={() => setGradeActiveIndex(idx)} onMouseLeave={() => setGradeActiveIndex(null)}>
                              <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full" style={{ backgroundColor: GRADE_COLORS[idx] }}></span>{d.label}</div>
                              <span className="font-bold">{d.value}</span>
                            </li>
                          ))}
                        </ul>
                              ...
                              <td className="px-6 py-4 text-sm">
                                <span className={`px-2 py-1 rounded text-xs font-medium ${sub.remark === 'Outstanding' ? 'bg-purple-100 text-purple-700' :
                                  sub.remark === 'Excellent' ? 'bg-green-100 text-green-700' :
                                    sub.remark === 'Good' ? 'bg-gray-100 text-accent' :
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                      <p className="text-xs text-accent font-bold uppercase">Percentage</p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white p-6 rounded-2xl shadow-sm">
                      <h3 className="font-bold text-gray-700 mb-4">Overall Attendance</h3>
                      <div className="h-56 flex items-center">
                        <div className="flex-1 h-full flex items-center justify-center">
                          <SimplePieChart
                            data={[{ label: 'Present', value: 93 }, { label: 'Absent', value: 7 }]}
                            colors={ATTENDANCE_COLORS}
                            id="attendance-chart"
                            activeIndex={attendanceActiveIndex}
                            onHover={setAttendanceActiveIndex}
                        ...
                        <div className="w-32 pl-2">
                          <ul className="space-y-1">
                            {[{ label: 'Present', value: 93 }, { label: 'Absent', value: 7 }].map((d, idx) => (
                              <li key={idx} className={`text-xs flex items-center justify-between p-1 rounded cursor-pointer ${attendanceActiveIndex === idx ? 'bg-gray-50' : ''}`} onMouseEnter={() => setAttendanceActiveIndex(idx)} onMouseLeave={() => setAttendanceActiveIndex(null)}>
                                <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full" style={{ backgroundColor: ATTENDANCE_COLORS[idx] }}></span>{d.label}</div>
                                <span className="font-bold">{d.value}%</span>
                              </li>
              ...
              {activeSection === 'Fee Payment' && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white p-6 rounded-2xl shadow-sm hover-card">
                      <h3 className="font-bold text-gray-900 mb-4">Fee Component Breakdown</h3>
                      <div className="h-56 flex items-center">
                        <div className="flex-1 h-full flex items-center justify-center">
                          <SimplePieChart
                            data={feeBreakdownData}
                            colors={FEE_COLORS}
                            id="fee-payment-trend-chart"
                            activeIndex={feeActiveIndex}
                            onHover={setFeeActiveIndex}
                          />
                        </div>
              ...
              {activeSection === 'Report Cards' && (
                <div className="space-y-8">
                  ...
                  <div className="bg-white rounded-2xl shadow-sm p-6 hover-card">
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {filteredAssignments.map((assignment) => (
                      <div key={assignment.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
              ...
              {activeSection === 'Exam Schedule' && (
                <div className="space-y-8">
                  ...
                  <div className="bg-white rounded-2xl shadow-sm overflow-hidden hover-card">
                    <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 bg-gray-200 rounded-2xl h-96 flex items-center justify-center relative overflow-hidden">
                      <BusRoute
                        ...
                        onSetHome={handleSetHomeLocation}
                      ...
                      <div className="bg-white p-6 rounded-2xl shadow-sm hover-card">
                        ...
                        <div className="space-y-4 relative">
                          ...
                          <div className="relative pl-6">
                            ...
                            <p className="text-xs text-gray-500">Departed 2:30 PM • On Time</p>
                          </div>
                          <div className="relative pl-6">
                            ...
                            <p className="text-xs text-indigo-600 font-medium">Lat: {busLocation.lat.toFixed(4)}, Lng: {busLocation.lng.toFixed(4)}</p>
                          </div>
                          <div className="relative pl-6">
                            ...
                            <p className="text-xs text-gray-500">Est. 3:15 PM • {busLocation.eta} remaining</p>
                          </div>
                        </div>
                        <button
                          onClick={() => setIsSettingHome(!isSettingHome)}
                          className={`w-full mt-4 py-2 rounded-lg font-medium text-sm transition-colors ${isSettingHome ? 'bg-red-50 text-red-600 hover:bg-red-100' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'}`}
                        >
                          {isSettingHome ? 'Cancel Setting Home' : 'Set Home Location'}
                        </button>
                      </div>
                    </div>
                  </div>
              ...
              {activeSection === 'Parent Profile' && (
                <div className="max-w-4xl mx-auto">
                  <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover-card">
                    ...
                    <div className="space-y-4">
                      {notifications.map((notification) => {
                        ...
                        switch (notification.type) {
                          case 'fee':
                            icon = <Bell size={20} />;
                            styles = { bg: 'bg-gray-50', border: 'border-gray-100', iconBg: 'bg-gray-100', iconColor: 'text-accent' };
                            break;
                          case 'assignment':
                    ...
                    <div className="px-8 pb-8">
                      <div className="relative flex justify-between items-end -mt-12 mb-8">
                        <div className="flex items-end gap-6">
                          <div className="w-24 h-24 rounded-full border-4 border-white bg-white shadow-md flex items-center justify-center text-3xl font-bold text-accent">
                            {getStudentDetails().name.charAt(0)}
                          </div>
                          <img
                            src={getStudentDetails().profileImage || `https://ui-avatars.com/api/?name=${encodeURIComponent(getStudentDetails().name)}&background=random&color=fff&size=128`}
                            alt={getStudentDetails().name}
                            className="w-24 h-24 rounded-full border-4 border-white shadow-md object-cover bg-white"
                            onError={(e) => {
                              e.target.onerror = null;
                              e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(getStudentDetails().name)}&background=random&color=fff&size=128`;
                            }}
                          />
