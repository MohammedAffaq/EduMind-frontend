...
const ContactItem = ({ contact, isSelected, onClick }) => (
  <div
    ...
    className={`flex items-center p-4 cursor-pointer transition-colors border-l-4 ${
      isSelected ? 'bg-gray-50 border-accent' : 'border-transparent hover:bg-gray-50'
    }`}
  >
...
const MessageBubble = ({ message, onReply, onForward }) => {
  ...
  return (
    <div className={`flex my-2 ${isOutgoing ? 'justify-end' : 'justify-start'} group items-center`}>
      ...
      >
        {message.isForwarded && (
          <div className={`flex items-center gap-1 text-xs mb-1 italic ${isOutgoing ? 'text-green-100' : 'text-accent'}`}>
            <Forward size={12} /> Forwarded
          </div>
        )}
        {message.replyTo && (
          <div className={`mb-2 p-2 rounded border-l-4 text-xs ${isOutgoing ? 'bg-green-600 border-green-800 text-green-100' : 'bg-gray-100 border-accent text-accent'}`]}]}**
        ...
        <div className="p-4 border-b border-gray-200/80">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-gray-100 border-transparent rounded-xl focus-accent transition-all"
            />
          </div>
        </div>
        {/* Contacts list */}
        ...
        <header className="bg-white/80 backdrop-blur-lg border-b border-gray-200/80 p-4 flex items-center justify-between shadow-sm">
          ...
          {selectedContact && (
            <div className="flex items-center gap-2">
              <button className="p-3 text-gray-500 hover:bg-gray-100 hover:text-accent rounded-full transition-colors"><Phone size={20} /></button>
              <button className="p-3 text-gray-500 hover:bg-gray-100 hover:text-accent rounded-full transition-colors"><Video size={20} /></button>
              <div className="relative">
                <button onClick={() => setShowOptions(!showOptions)} className="p-3 text-gray-500 hover:bg-gray-100 hover:text-accent rounded-full transition-colors"><MoreVertical size={20} /></button>
                {showOptions && (
          ...
          <footer className="bg-white/80 backdrop-blur-lg border-t border-gray-200/80 p-4">
            {replyingTo && (
              <div className="flex items-center justify-between bg-gray-50 p-2 px-4 rounded-lg mb-2 border border-gray-200">
                <div className="text-sm">
                  <span className="text-accent font-semibold">Replying to {replyingTo.sender === 'me' ? 'yourself' : selectedContact.name}</span>
            ...
            <form onSubmit={handleSendMessage} className="flex items-center gap-4">
              ...
              <input
                ...
                className="flex-1 px-5 py-3 bg-gray-100 border-transparent rounded-full focus-accent"
              />
              <button type="submit" className="p-3 bg-accent text-white rounded-full hover:opacity-95 transition-colors disabled:bg-gray-300 shadow-lg" disabled={!inputText.trim()}>
                <Send size={20} />
              </button>
            </form>
          </footer>
