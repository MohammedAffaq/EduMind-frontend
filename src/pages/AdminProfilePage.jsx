    ...
    const fetchAdminData = async () => {
      ...
      if (currentUser && currentUser.firstName) {
        setProfileData(prev => ({
          ...
          role: currentUser.role === 'admin' ? 'Administrator' : currentUser.role || prev.role,
        }));
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Also load profile image if it exists
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/admin/profile', {
        ...
        body: JSON.stringify({
          ...
          profilePicture: profileImage // Send the base64 string
        })
      });
