...
import { LayoutDashboard, GraduationCap, Users, Bus, DollarSign, CalendarCheck, LogOut, Search, Menu, Bell, User, FileText, Image, Clock, Settings, ShieldCheck, UserCog, Wrench } from 'lucide-react';

const NotificationItem = ({ avatar, name, action, target, time, type, attachment, read }) => (
  <div className={`flex gap-4 p-4 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors rounded-xl group ${!read ? 'bg-gray-50/40' : ''}`}>
    <div className="flex-shrink-0 relative">
      ...
      {!read && <span className="absolute -top-1 -right-1 w-3 h-3 bg-accent border-2 border-white rounded-full"></span>} 
    </div>
...
const NavItem = ({ icon, label, active, onClick, badge }) => (
  <button onClick={onClick} className={`flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-200 group w-full ${active ? 'bg-indigo-50 text-indigo-600 font-bold' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900 font-medium'}`}>
